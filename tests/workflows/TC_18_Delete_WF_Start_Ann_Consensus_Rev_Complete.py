...
def test_tc18_delete_wf_start_ann_consensus_rev_complete(before_each):
    ...
    try:
        ...
        if project_name in project_name_list:
            ...
            action_factory.helpers.attach_screenshot(name="TC18ProjectCreated")
            action_factory.helpers.attach_allure(name="TC_18 Project", text=message)
            assert True, message
        else:
            status = "Fail"
            message = f"Project '{project_name}' was not created."
            action_factory.helpers.attach_screenshot(name="TC18ProjectNotCreated")
            action_factory.helpers.attach_allure(name="TC_18 Project", text=message)
        ...
        if error_visible:
            ...
            action_factory.helpers.attach_allure(name="TC_18 Delete Linked Workflow", text=message)
            assert True, message
        else:
            status = "Fail"
            message = f"Deletion of linked workflow '{workflow_name}' was not blocked."
            action_factory.helpers.attach_screenshot(name="TC18PreventDeleteLinkedWorkflowFailed")
            action_factory.helpers.attach_allure(name="TC_18 Delete Linked Workflow", text=message)
            assert False, message
        ...
        if workflow_name in workflow_name_list:
            ...
            action_factory.helpers.attach_screenshot(name="TC18WorkflowNotDeleted")
            action_factory.helpers.attach_allure(name="TC_18 Delete Workflow", text=message)
            assert True, message
        else:
            status = "Fail"
            message = f"Workflow '{workflow_name}' was deleted successfully."
            action_factory.helpers.attach_screenshot(name="TC18DeleteWorkflowFailed")
            action_factory.helpers.attach_allure(name="TC_18 Delete Workflow", text=message)
            assert False, message
