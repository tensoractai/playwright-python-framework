...
def test_tc19_edit_wf_start_ann1_ann2_consensus_rev_complete(before_each):
    ...
    try:
        ...
        if workflow_name in workflow_list:
            ...
            action_factory.helpers.attach_screenshot(name="TC19WorkflowCreationPass")
            action_factory.helpers.attach_allure(name="TC_19 Create Workflow", text=message)
            assert True, message
        else:
            status = "Fail"
            message = f"Workflow '{workflow_name}' not found after creation."
            action_factory.helpers.attach_screenshot(name="TC19WorkflowCreationFailed")
            action_factory.helpers.attach_allure(name="TC_19 Create Workflow", text=message)
