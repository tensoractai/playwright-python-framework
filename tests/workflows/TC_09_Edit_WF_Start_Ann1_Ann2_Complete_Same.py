...
def test_tc09_edit_wf_start_ann1_ann2_complete_same(before_each):
    ...
    try:
        ...
        if workflow_name in workflow_list:
            ...
            action_factory.helpers.attach_screenshot(name="TC09WorkflowCreationPass")
            action_factory.helpers.attach_allure(name="TC_09 Create Workflow", text=message)
            assert True, message
        else:
            status = "Fail"
            message = f"Workflow '{workflow_name}' not found after creation."
            action_factory.helpers.attach_screenshot(name="TC09WorkflowCreationFailed")
            action_factory.helpers.attach_allure(name="TC_09 Create Workflow", text=message)
