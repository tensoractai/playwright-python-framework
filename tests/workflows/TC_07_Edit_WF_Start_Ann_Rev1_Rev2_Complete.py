...
def test_tc07_edit_wf_start_ann_rev1_rev2_complete(before_each):
    ...
    try:
        ...
        if workflow_name in workflow_list:
            ...
            action_factory.helpers.attach_screenshot(name="TC07WorkflowCreationPass")
            action_factory.helpers.attach_allure(name="TC_07 Create Workflow", text=message)
            assert True, message
        else:
            status = "Fail"
            message = f"Workflow '{workflow_name}' not found after creation."
            action_factory.helpers.attach_screenshot(name="TC07WorkflowCreationFailed")
            action_factory.helpers.attach_allure(name="TC_07 Create Workflow", text=message)
        ...
        # Connect new nodes
        action_factory.workflows_actions.nodes_connection_flow(
            node_Name1="review", node_index1=3, position1="right", index1=2,
            node_Name2="review", node_index2=2, position2="left", index2=1
        )
        action_factory.ui_utils.smart_wait()
