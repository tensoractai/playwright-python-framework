...
def test_tc08_delete_wf_start_ann_rev1_rev2_complete(before_each):
    ...
    try:
        ...
        if project_name in project_name_list:
            ...
            action_factory.helpers.attach_screenshot(name="TC08ProjectCreated")
            action_factory.helpers.attach_allure(name="TC_08 Project", text=message)
            assert True, message
        else:
            status = "Fail"
            message = f"Project '{project_name}' was not created."
            action_factory.helpers.attach_screenshot(name="TC08ProjectNotCreated")
            action_factory.helpers.attach_allure(name="TC_08 Project", text=message)
        ...
        if workflow_name in workflow_name_list:
            ...
            action_factory.helpers.attach_screenshot(name="TC08WorkflowNotDeleted")
            action_factory.helpers.attach_allure(name="TC_08 Delete Workflow", text=message)
            assert True, message
        else:
            status = "Fail"
            message = f"Workflow '{workflow_name}' was deleted successfully."
            action_factory.helpers.attach_screenshot(name="TC08DeleteWorkflowFailed")
            action_factory.helpers.attach_allure(name="TC_08 Delete Workflow", text=message)
            assert False, message
