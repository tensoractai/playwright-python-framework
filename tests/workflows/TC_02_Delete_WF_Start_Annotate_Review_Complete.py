...
@allure.story("TC_02: Start -> Annotate -> Review -> Complete (Delete Linked)")
@allure.title("Verify attempting to delete a TC_02 workflow linked to an active project is blocked")
def test_tc02_delete_wf_start_annotate_review_complete(before_each):
    status = "Fail"
    message = ""
    try:
        ...
        project_name = test_data_inputs.tc02_project_name
        ...
        if project_name in project_name_list:
            ...
            action_factory.helpers.attach_screenshot(name="TC02ProjectCreated")
            action_factory.helpers.attach_allure(name="TC_02 Project", text=message)
            assert True, message
        else:
            status = "Fail"
            message = f"Project '{project_name}' was not created."
            action_factory.helpers.attach_screenshot(name="TC02ProjectNotCreated")
            action_factory.helpers.attach_allure(name="TC_02 Project", text=message)
        ...
        if error_visible:
            ...
            action_factory.helpers.attach_screenshot(name="TC02PreventDeleteLinkedWorkflowPass")
            action_factory.helpers.attach_allure(name="TC_01 Delete Linked Workflow", text=message)
            assert True, message
        else:
            status = "Fail"
        ...
        if workflow_name in workflow_name_list:
            ...
            action_factory.helpers.attach_screenshot(name="TC02WorkflowNotDeleted")
            action_factory.helpers.attach_allure(name="TC_02 Delete Workflow", text=message)
            assert True, message
        else:
            status = "Fail"
            message = f"Workflow '{workflow_name}' was deleted successfully."
            action_factory.helpers.attach_screenshot(name="TC02DeleteWorkflowFailed")
            action_factory.helpers.attach_allure(name="TC_02 Delete Workflow", text=message)
            assert False, message
