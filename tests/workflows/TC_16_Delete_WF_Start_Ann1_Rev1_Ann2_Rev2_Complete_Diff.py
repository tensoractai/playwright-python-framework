...
def test_tc16_delete_wf_start_ann1_rev1_ann2_rev2_complete_diff(before_each):
    ...
    try:
        ...
        if project_name in project_name_list:
            ...
            action_factory.helpers.attach_screenshot(name="TC16ProjectCreated")
            action_factory.helpers.attach_allure(name="TC_16 Project", text=message)
            assert True, message
        else:
            status = "Fail"
            message = f"Project '{project_name}' was not created."
            action_factory.helpers.attach_screenshot(name="TC16ProjectNotCreated")
            action_factory.helpers.attach_allure(name="TC_16 Project", text=message)
        ...
        if error_visible:
            ...
            action_factory.helpers.attach_allure(name="TC_16 Delete Linked Workflow", text=message)
            assert True, message
        else:
            status = "Fail"
            message = f"Deletion of linked workflow '{workflow_name}' was not blocked."
            action_factory.helpers.attach_screenshot(name="TC16PreventDeleteLinkedWorkflowFailed")
            action_factory.helpers.attach_allure(name="TC_16 Delete Linked Workflow", text=message)
            assert False, message
        ...
        if workflow_name in workflow_name_list:
            ...
            action_factory.helpers.attach_screenshot(name="TC16WorkflowNotDeleted")
            action_factory.helpers.attach_allure(name="TC_16 Delete Workflow", text=message)
            assert True, message
        else:
            status = "Fail"
            message = f"Workflow '{workflow_name}' was deleted successfully."
            action_factory.helpers.attach_screenshot(name="TC16DeleteWorkflowFailed")
            action_factory.helpers.attach_allure(name="TC_16 Delete Workflow", text=message)
            assert False, message
