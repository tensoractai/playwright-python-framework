...
def test_tc10_delete_wf_start_ann1_ann2_complete_same(before_each):
    ...
    try:
        ...
        if project_name in project_name_list:
            ...
            action_factory.helpers.attach_screenshot(name="TC10ProjectCreated")
            action_factory.helpers.attach_allure(name="TC_10 Project", text=message)
            assert True, message
        else:
            status = "Fail"
            message = f"Project '{project_name}' was not created."
            action_factory.helpers.attach_screenshot(name="TC10ProjectNotCreated")
            action_factory.helpers.attach_allure(name="TC_10 Project", text=message)
        ...
        if workflow_name in workflow_name_list:
            ...
            action_factory.helpers.attach_screenshot(name="TC10WorkflowNotDeleted")
            action_factory.helpers.attach_allure(name="TC_10 Delete Workflow", text=message)
            assert True, message
        else:
            status = "Fail"
            message = f"Workflow '{workflow_name}' was deleted successfully."
            action_factory.helpers.attach_screenshot(name="TC10DeleteWorkflowFailed")
            action_factory.helpers.attach_allure(name="TC_10 Delete Workflow", text=message)
            assert False, message
