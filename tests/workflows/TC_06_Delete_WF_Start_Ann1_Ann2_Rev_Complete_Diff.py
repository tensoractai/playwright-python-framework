...
def test_tc06_delete_wf_start_ann1_ann2_rev_complete_diff(before_each):
    ...
    try:
        ...
        if project_name in project_name_list:
            ...
            action_factory.helpers.attach_screenshot(name="TC06ProjectCreated")
            action_factory.helpers.attach_allure(name="TC_06 Project", text=message)
            assert True, message
        else:
            status = "Fail"
            message = f"Project '{project_name}' was not created."
            action_factory.helpers.attach_screenshot(name="TC06ProjectNotCreated")
            action_factory.helpers.attach_allure(name="TC_06 Project", text=message)
        ...
        if workflow_name in workflow_name_list:
            ...
            action_factory.helpers.attach_screenshot(name="TC06WorkflowNotDeleted")
            action_factory.helpers.attach_allure(name="TC_06 Delete Workflow", text=message)
            assert True, message
        else:
            status = "Fail"
            message = f"Workflow '{workflow_name}' was deleted successfully."
            action_factory.helpers.attach_screenshot(name="TC06DeleteWorkflowFailed")
            action_factory.helpers.attach_allure(name="TC_06 Delete Workflow", text=message)
            assert False, message
